package net.spizzer.aoc2019.intcode;

import org.junit.jupiter.api.Assertions;
import org.junit.jupiter.api.Test;

import static org.junit.jupiter.api.Assertions.assertEquals;

class EarlyIntcodeComputerTest {

    @Test
    void testIntcodeIO() {
        long[] program = new long[]{3,0,4,0,99};
        Assertions.assertEquals(42, runIO(program, 42));
    }

    @Test
    void testIntcodeEqualPosition() {
        long[] program = new long[]{3,9,8,9,10,9,4,9,99,-1,8};
        assertEquals(1, runIO(program, 8));
        assertEquals(0, runIO(program, 7));
        assertEquals(0, runIO(program, 9));
    }

    @Test
    void testIntcodeLessThanPosition() {
        long[] program = new long[]{3,9,7,9,10,9,4,9,99,-1,8};
        assertEquals(0, runIO(program, 8));
        assertEquals(1, runIO(program, 7));
        assertEquals(0, runIO(program, 9));
    }

    @Test
    void testIntcodeEqualImmediate() {
        long[] program = new long[]{3,3,1108,-1,8,3,4,3,99};
        assertEquals(1, runIO(program, 8));
        assertEquals(0, runIO(program, 7));
        assertEquals(0, runIO(program, 9));
    }

    @Test
    void testIntcodeLessThanImmediate() {
        long[] program = new long[]{3, 3, 1107, -1, 8, 3, 4, 3, 99};
        assertEquals(0, runIO(program, 8));
        assertEquals(1, runIO(program, 7));
        assertEquals(0, runIO(program, 9));
    }

    @Test
    void testIntcodeNonZeroPosition() {
        long[] program = new long[]{3,12,6,12,15,1,13,14,13,4,13,99,-1,0,1,9};
        assertEquals(0, runIO(program, 0));
        assertEquals(1, runIO(program, 1));
        assertEquals(1, runIO(program, -1));
    }

    @Test
    void testIntcodeNonZeroImmediate() {
        long[] program = new long[]{3,3,1105,-1,9,1101,0,0,12,4,12,99,1};
        assertEquals(0, runIO(program, 0));
        assertEquals(1, runIO(program, 1));
        assertEquals(1, runIO(program, 2));
    }

    @Test
    void testIntcodeEight() {
        long[] program = new long[]{3,21,1008,21,8,20,1005,20,22,107,8,21,20,1006,20,31,
                1106,0,36,98,0,0,1002,21,125,20,4,20,1105,1,46,104,
                999,1105,1,46,1101,1000,1,20,4,20,1105,1,46,98,99};
        assertEquals(999, runIO(program, 7));
        assertEquals(1000, runIO(program, 8));
        assertEquals(1001, runIO(program, 9));
    }

    private static long runIO(long[] instructions, long input) {
        IntcodeComputer computer = new IntcodeComputer(instructions);
        computer.run(input);
        return computer.getLastOutput();
    }

}